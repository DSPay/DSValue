...
int64_t CCoinsView::GetTotalCoinsValues(const std::string& address){return 0;}
...
int64_t CCoinsViewCache::GetTotalCoinsValues(const std::string& address)
{
	int64_t values = 0;
	txnouttype type;
	...
	BOOST_FOREACH(const PAIRTYPE(uint256,CCoins) &item, cacheCoins)
	{
		if (base->HaveCoins(item.first))
		{
			continue;
		}
		for (unsigned int i = 0; i < item.second.vout.size(); ++i)
		{
			if(ExtractDestinations(item.second.vout[i].scriptPubKey, type, addresses, nRequired))
			{
	...
	values += base->GetTotalCoinsValues(address);
	return values;
}
