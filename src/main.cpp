...
int64_t GetLottoValueFromBlock(const CBlock &block)
{
	...
	BOOST_FOREACH(const CTransaction &tx, block.vtx) {
		nCurBetValue += GetLottoValueFromTx(tx);
	}
	return nCurBetValue;
}
...
bool GetLotteryKey(int nHeight, int &nID, uint256 &lotterykey) {
	...
	if (ret) {
		lotterykey = lottoFileKey.getKey()[0];
	}
	LogPrintf("Get nLottoId=%d, nLottoKey=%s\n", nID, lotterykey.GetHex());
	return ret;
}
...
int64_t GetLottoFee(const CTransaction& tx) {
	int64_t llLottoValue = 0;
	TransactionState iTxType(NORMAL_STATE);
	BOOST_FOREACH(const CTxOut &vOut, tx.vout) {
		if (vOut.GetLottoTxOutType(iTxType)) {
			if(iTxType == VALID_LOTTO_STATE)
				llLottoValue += vOut.nValue;
		}
	}
	return (llLottoValue * g_ldLottoFeeRate);
}
...
bool CheckTransaction(const CTransaction& tx, CValidationState &state) {
	...
	BOOST_FOREACH(const CTxIn& txin, tx.vin) {
		if (vInOutPoints.count(txin.prevout))
			return state.DoS(100, error("CheckTransaction() : duplicate inputs"), REJECT_INVALID,
	...
	if (tx.IsCoinBase()) {
		...
		BOOST_FOREACH(const CTxIn& txin, tx.vin)
			if (txin.prevout.IsNull())
				return state.DoS(10, error("CheckTransaction() : prevout is null"), REJECT_INVALID,
...
int64_t GetMinFee(const CTransaction& tx, unsigned int nBytes, bool fAllowFree, enum GetMinFee_mode mode) {
	...
	if (!MoneyRange(nMinFee))
		nMinFee = MAX_MONEY;
	int64_t llLottoFee = GetLottoFee(tx);
	nMinFee += llLottoFee;
	return nMinFee;
}
...
bool AcceptToMemoryPool(CTxMemPool& pool, CValidationState &state, const CTransaction &tx, bool fLimitFree,
	...
	if (Params().NetworkID() == CChainParams::MAIN && !IsStandardTx(tx, reason))
		return state.DoS(0, error("AcceptToMemoryPool : nonstandard transaction: %s", reason), REJECT_NONSTANDARD,
				reason);
...

	{
...

		{
			...
			if (!view.HaveInputs(tx))
				return state.Invalid(error("AcceptToMemoryPool : inputs already spent"), REJECT_DUPLICATE,
		...
		//check inputvalue and outputvalue
		unsigned int ret = 0;
		if (tx.IsTransactionValid()) {
			if (nFees < (GetLottoFee(tx)))
				return error("AcceptToMemoryPool: :lotto fee is not correct");
		} else {
			return error("AcceptToMemoryPool: :transation is invalid");
		}
		...
		if (fLimitFree && nFees < txMinFee)
			return state.DoS(0,
					error("AcceptToMemoryPool : not enough fees %s, %d < %d", hash.ToString(), nFees, txMinFee),
		...
		if (fRejectInsaneFee && nFees > CTransaction::nMinRelayTxFee * 10000)
			return error("AcceptToMemoryPool: : insane fees %s, %d > %d", hash.ToString(), nFees,
					CTransaction::nMinRelayTxFee * 10000);
...
int64_t GetBlockValue(int nHeight, int64_t nFees) {
    ...
    else if(nHeight < 105121)
    {
    	nSubsidy = 250 * COIN;
    }
    else if(nHeight < 210240)
    {
    	nSubsidy = 130 * COIN;
    }
...
void CheckForkWarningConditions() {
	...
	if(pindexBestForkTip && chainActive.Contains(pindexBestForkTip)){
		pindexBestForkTip = NULL;
	}
	...
	if (pindexBestForkTip
			|| (pindexBestInvalid
					&& pindexBestInvalid->nChainWork
							> chainActive.Tip()->nChainWork + (chainActive.Tip()->GetBlockWork() * 6).getuint256())) {
...
bool CheckInputs(const CTransaction& tx, CValidationState &state, CCoinsViewCache &inputs, bool fScriptChecks,
	...
	if (!tx.IsCoinBase()) {
		...
		for (unsigned int i = 0; i < tx.vin.size(); i++) {
			...
			if (!MoneyRange(coins.vout[prevout.n].nValue) || !MoneyRange(nValueIn))
				return state.DoS(100, error("CheckInputs() : txin values out of range"), REJECT_INVALID,
		...
		if (nTxFee < 0)
			return state.DoS(100, error("CheckInputs() : %s nTxFee < 0", tx.GetHash().ToString()), REJECT_INVALID,
					"bad-txns-fee-negative");
		nFees += nTxFee;
		if (!MoneyRange(nFees))
			return state.DoS(100, error("CheckInputs() : nFees out of range"), REJECT_INVALID,
...
bool ConnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck) {
	...
	if(!CheckLottoResult(block, state))
		return false;

	// verify that the view's current state corresponds to the previous block
	uint256 hashPrevBlock = pindex->pprev == NULL ? uint256(0) : pindex->pprev->GetBlockHash();
	if(hashPrevBlock != view.GetBestBlock()){
		LogPrintf("ConnectBlock(), prev block hash:%s\n", hashPrevBlock.GetHex());
		LogPrintf("ConnectBlock(), best block hash:%s\n", view.GetBestBlock().GetHex());
		return state.DoS(100,error("ConnectBlock():Prev block hash do not same with best block\n"),
						REJECT_INVALID, "bad-pre-hash", true);
	}
	...
	unsigned int flags = SCRIPT_VERIFY_NOCACHE | (fStrictPayToScriptHash ? SCRIPT_VERIFY_P2SH : SCRIPT_VERIFY_NONE);
	...
	if (fBenchmark)
		LogPrintf("- Connect %u transactions: %.2fms (%.3fms/tx, %.3fms/txin)\n", (unsigned )block.vtx.size(),
				0.001 * nTime, 0.001 * nTime / block.vtx.size(), nInputs <= 1 ? 0 : 0.001 * nTime / (nInputs - 1));
	...
	if (block.vtx[0].vout[0].nValue > GetBlockValue(pindex->nHeight, nFees))
		return state.DoS(100,
				error("ConnectBlock() : coinbase pays too much (actual=%d vs limit=%d)", block.vtx[0].GetValueOut(),
						GetBlockValue(pindex->nHeight, nFees)), REJECT_INVALID, "bad-cb-amount");

	if (!control.Wait())
		return state.DoS(100, false);
	int64_t nTime2 = GetTimeMicros() - nStart;
	if (fBenchmark)
		LogPrintf("- Verify %u txins: %.2fms (%.3fms/txin)\n", nInputs - 1, 0.001 * nTime2,
...
bool CheckBTCHashValid(CBlockIndex *pIndex) {
...
bool CheckLastBlockState(int nCurBTCHeight, CValidationState& state) {
	if (nCurBTCHeight <= BTC_START_HIGHT) {
		return true;
	}
	...
	} while (pindexBlock = chainActive.Next(pindexBlock));
	return true;
}
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot) {
	...
	if (block.GetBlockTime() > GetAdjustedTime() + 2 * 60 * 60)
		return state.Invalid(error("CheckBlock() : block timestamp too far in the future"), REJECT_INVALID,
				"time-too-new");
...
bool CheckLottoResult(const CBlock& block, CValidationState& state){
	...
	uint256 pBlockHash = block.hashPrevBlock;
	map<uint256, CBlockIndex*>::iterator mi = mapBlockIndex.find(pBlockHash);
	if (mi == mapBlockIndex.end()) {
		//this is an orphan block, which will be put into COrphanBlock map.
		return true;
	}
	...
	CBlock pPreBlock;
	CDiskBlockPos diskBlockPos(pPreBlockIndex->nFile, pPreBlockIndex->nDataPos);
	if (!ReadBlockFromDisk(pPreBlock, diskBlockPos))
		return state.DoS(100, error("CheckLottoResult() : read lottery block from disk failed"), REJECT_INVALID_LOTTO,
				"bad-previous-block", true);

	Bitcoin::CChainManager &chainManager = Bitcoin::CChainManager::CreateChainManagerInstance();
	lotto::CLottoHeader preLottoHeader = pPreBlock.lottoHeader;
	if (lottoHeader.GetFirstBitcoinHeight() - preLottoHeader.GetLastBitcoinHeight() > 1) {
		return state.DoS(100,
				error(
	...
	if (lottoHeader.mBitcoinHash.begin()->first != BTC_START_HIGHT) {
		...
		CBlockIndex *pBitcoinIndex = chainManager.GetBitcoinBlockIndex(lottoHeader.mBitcoinHash.begin()->first);
		//can't judge the bitcoin hash in the block, put the block into the vCandidateBlock vector
		if (NULL == pBitcoinIndex) {
			return state.DoS(0,
					error(
						"CheckLottoResult() : BTC hash in lottoheader can't get block index"),
						REJECT_INVALID_LOTTO, "bad-btc-hash-sequence", true);
		}
		if (((NULL != pBitcoinIndex) && preLottoHeader.GetLastBitcoinHash() != *(pBitcoinIndex->phashBlock))
				&& ((NULL != pPreBitcoinIndex) && preLottoHeader.GetLastBitcoinHash() != *(pPreBitcoinIndex->phashBlock))) {
			return state.DoS(100,
					error(
							"CheckLottoResult() : BTC hash in lottoheader do not followed by a sequence after the previous block's lottoheader"),
							REJECT_INVALID_LOTTO, "bad-btc-hash-sequence", true);
		}
	}
	uint256 iterPreBTCHash = uint256(0);
	for (std::map<int, uint256>::const_iterator iterBTCHash = block.lottoHeader.mBitcoinHash.begin();
			iterBTCHash != block.lottoHeader.mBitcoinHash.end(); ++iterBTCHash) {
		CBlockIndex *pBitcoinIndex = chainManager.GetBitcoinBlockIndex(iterBTCHash->first);
		if (NULL == pBitcoinIndex) {
			return state.DoS(0,
					error(
						"CheckLottoResult() : do not have the BTC's block index which's hash contained in lottohead,"
						"btchash=%s ,which block hash=%s",iterBTCHash->second.GetHex(), block.GetHash().GetHex()),
						REJECT_INVALID_LOTTO, "bad-btc-hash-sequence", true);
		}
		if ((0 != iterPreBTCHash && NULL != pBitcoinIndex->pprev
	...
	int64_t nCurBetValue = GetLottoValueFromBlock(block);

	//Check CoinBase is correct
	if (pPreBlockIndex->nHeight >= 2 * nIntervalLottery && uint256(0) != lottoHeader.uLottoKey) {
		CTransaction coinBaseTx = block.vtx[0];
		std::vector<CTxOut> lottoTxOut(coinBaseTx.vout.begin() + 1, coinBaseTx.vout.end());
		int64_t llPool = pPreBlock.lottoHeader.llPool;
		std::vector<CTxOut> vTxOut;
		int64_t llRewards = 0;

		int nID = 0;
		uint256 lottoKey(0);
		if (!GetLotteryKey(pPreBlockIndex->nHeight, nID, lottoKey)) {
            //required the lotto key from node
			{
				LOCK(cs_vNodes);
				BOOST_FOREACH(CNode* pnode, vNodes)
					pnode->PushMessage("getlottokey", nID);
			}
			LogPrintf("CheckLottoResult(): can't get lotto key\n");
			//return true;
			return state.DoS(0, error("CheckLottoResult(): can't get lotto key"), REJECT_INVALID_LOTTO,
					"check lotto result error", true);
		}
		LogPrintf("nID:%d,uLottoKey:%s\n", nID, lottoKey.ToString());
		if (lottoKey != block.lottoHeader.uLottoKey || nID != block.lottoHeader.nLottoID) {
...
bool AcceptBlock(CBlock& block, CValidationState& state, CDiskBlockPos* dbp) {
	...
	if (hash != Params().HashGenesisBlock()) {
		...
		{
			LogPrintf("Check proof of work error\n");
			block.print();
			LogPrintf("NextWorkRequired bits:%08x", GetNextWorkRequired(pindexPrev, &block));
		...
		BOOST_FOREACH(const CTransaction& tx, block.vtx)
			if (!IsFinalTx(tx, nHeight, block.GetBlockTime()))
				return state.DoS(10, error("AcceptBlock() : contains a non-final transaction"), REJECT_INVALID,
						"bad-txns-nonfinal");
		...
		if (block.nVersion >= 2) {
			...
			if ((!TestNet() && CBlockIndex::IsSuperMajority(2, pindexPrev, 750, 1000))
				...
				if (block.vtx[0].vin[0].scriptSig.size() < expect.size()
					...
					return state.DoS(100, error("AcceptBlock() : block height mismatch in coinbase"), REJECT_INVALID,
							"bad-cb-height");
			}
		}
	}
...
bool ProcessBlock(CValidationState &state, CNode* pfrom, CBlock* pblock, CDiskBlockPos *dbp) {
	...
	if (mapBlockIndex.count(hash))
		return state.Invalid(
				error("ProcessBlock() : already have block %d %s", mapBlockIndex[hash]->nHeight, hash.ToString()), 0,
				"duplicate");
	if (mapOrphanBlocks.count(hash))
		return state.Invalid(error("ProcessBlock() : already have block (orphan) %s", hash.ToString()), 0, "duplicate");

	if(!CheckLastBlockState(pblock->lottoHeader.GetLastBitcoinHeight(), state)){
		return error("ProcessBlock() : CheckLastBlockState FAILED");
	}
...
bool static LoadBlockIndexDB() {
	...
	chainActive.Genesis()->print();
//    LogTrace("test","chainActive genesis:%s", chainActive.Genesis()->GetBlockHash().ToString().c_str());
...
bool VerifyDB(int nCheckLevel, int nCheckDepth) {
	...
	for (CBlockIndex* pindex = chainActive.Tip(); pindex && pindex->pprev; pindex = pindex->pprev) {
		...
		if (nCheckLevel >= 3 && pindex == pindexState
				&& (coins.GetCacheSize() + pcoinsTip->GetCacheSize()) <= 2 * nCoinCacheSize + 32000) {
			bool fClean = true;
...
void PrintBlockTree() {
	...
	while (!vStack.empty()) {
		...
		LogPrintf("%d (blk%05u.dat:0x%x)  %s  tx %"PRIszu"", pindex->nHeight, pindex->GetBlockPos().nFile,
				pindex->GetBlockPos().nPos, DateTimeStrFormat("%Y-%m-%d %H:%M:%S", block.GetBlockTime()),
				block.vtx.size());
	...
	else if (strCommand == "lotterykey")
	{
		...
		BOOST_FOREACH(SyncData::CSyncData& data, vdata)
		{
			if (data.CheckSignature(SyncData::strSyncDataPubKey))
			{
				...
				sstream >> key;
				if (!db.WriteLotteryKey(key.Index, data))
				{
					LogPrintf("WriteLotteryKey error key index: %d\n", key.Index);
					continue;
				}
				pfrom->setLotteKeyKnown.insert(key.Index);
				vIndex.push_back(key.Index);
			}
		}
		if (!vIndex.empty())
		{
		...
		if (vIndex.size() == 1 && vIndex.size() == vdata.size())
		{
			...
			BOOST_FOREACH(CNode* pnode, vNodes)
			{
				if (pnode->setLotteKeyKnown.count(index) == 0)
				{
					pnode->setLotteKeyKnown.insert(index);
					pnode->PushMessage("lotterykey", vdata);
				}
			}
		}
	}
	else if (strCommand == "getlottokey")
	{
		int index = 0;
		int maxId = 0;
		const int maxSendKeyNum = 200;
		vRecv >> index;
		SyncData::CSyncDataDb db;
		std::vector<SyncData::CSyncData> vdata;
		{
			...
			uint256 uLottoKey(0);
			GetLotteryKey(nHeight, maxId, uLottoKey);

		}
		while(index <= maxId && vdata.size() < maxSendKeyNum)
		{
			SyncData::CSyncData data;
			if (pfrom->setLotteKeyKnown.count(index) != 0
				|| !db.ReadLotteryKey(index, data))
			{
				index = index + 1;
				continue;
			}
			vdata.push_back(data);
			pfrom->setLotteKeyKnown.insert(index);
			index = index + 1;
		}
		if (!vdata.empty())
		{
			pfrom->PushMessage("lotterykey", vdata);
		}
	}
	else {
	...
	while (!pfrom->fDisconnect && it != pfrom->vRecvMsg.end()) {
		...
		if (nChecksum != hdr.nChecksum) {
			LogPrintf("ProcessMessages(%s, %u bytes) : CHECKSUM ERROR nChecksum=%08x hdr.nChecksum=%08x\n", strCommand,
					nMessageSize, nChecksum, hdr.nChecksum);
			continue;
		}
