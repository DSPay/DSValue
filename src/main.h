...
/** check last blocks which contains last 6 height BTC block hash, make the block don't contain wrong BTC hash.*/
bool CheckLastBlockState(int nCurBTCHeight, CValidationState& state);
/** check last block BTC hash is valid current.*/
bool CheckBTCHashValid(CBlockIndex * pBlockIndex);
/** Get Lotto Value from block **/
int64_t GetLottoValueFromBlock(const CBlock &block);
/** Get Lotto Value from Tx **/
int64_t GetLottoValueFromTx(const CTransaction &tx);

bool GetLotteryKey(int nHeight, int &nID, uint256 &uLottoKey);
...
public:
    ...
    {
        return
        strprintf("CBlockIndex(pprev=%p, nHeight=%d, merkle=%s, hashBlock=%s, nBits=%08x, nNonce=%u, lottohead=%s)",
            pprev, nHeight,
            hashMerkleRoot.ToString().c_str(),
            GetBlockHash().ToString().c_str(),
            nBits, nNonce,
            lottoHeader.ToString().c_str());
    }
