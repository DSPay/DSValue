...
int CNetMessage::print() const
{
	static const char* s_hexTable[256] =
    	...
    	int len = 0;
		string te = hdr.GetCommand();
		std::cout << DateTimeStrFormat("%Y-%m-%d %H:%M:%S", GetTime()) << '\n' ;
		std::cout << te ;
		string tem(vRecv.begin(), vRecv.end());
	  ...
	  //printf ("Current local time and date: %s", asctime(timeinfo));
		...
		te.append(asctime(timeinfo)).append("\n");
		BOOST_FOREACH(const char& item, tem) {
			if (0 == (len++ % 16)) {
				std::cout << '\n';
			}
			std::cout  << ':'<< s_hexTable[(const unsigned char)item];
			te.append(":").append(s_hexTable[(const unsigned char)item]);
		}
		std::cout <<'\n' <<'\n' << endl;
		//LogTrace("bess", "rece:\n%s\n",te.c_str());
		return 1;
