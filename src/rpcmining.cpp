...
Value getgenerate(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "It is set with the command line argument -gen (or dspay.conf setting gen)\n"
...
Value getwork(const Array& params, bool fHelp)
{
    ...
    if (params.size() == 0)
    {
            ...
            (mempool.GetTransactionsUpdated() != nTransactionsUpdatedLast && GetTime() - nStart > 60))
        {
            ...
            pblocktemplate = CreateNewBlockWithKey(*pMiningKey, state);
            if (!pblocktemplate)
...
Value getblocktemplate(const Array& params, bool fHelp)
{
        ...
        (mempool.GetTransactionsUpdated() != nTransactionsUpdatedLast && GetTime() - nStart > 5))
    {
        ...
        CValidationState state;
        pblocktemplate = CreateNewBlock(scriptDummy,state);
        if (!pblocktemplate)
            throw JSONRPCError(RPC_OUT_OF_MEMORY, "Out of memory");
    ...
    { //add by ranger.shi
 	 CDataStream sslottoHeader(SER_NETWORK, PROTOCOL_VERSION);
   	 sslottoHeader << pblock->lottoHeader;
   	 string strhead = HexStr(sslottoHeader.begin(),sslottoHeader.end());
     result.push_back(Pair("lottoHeader", strhead));
    }
