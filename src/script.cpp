...
bool CastToBool(const valtype& vch) {
	for (unsigned int i = 0; i < vch.size(); i++) {
		if (vch[i] != 0) {
			...
			if (i == vch.size() - 1 && vch[i] == 0x80) // Keeps only characters left of the specified point in a string. disabled.
				return false;
			return true;
		}
	}
...
const char* GetOpName(opcodetype opcode) {
	...
	case OP_LOTTO:
		return "OP_LOTTO";
	case OP_LOTTOUNSPEND:
		return "OP_LOTTOUNSPEND";
	default:
		return "OP_UNKNOWN";
	}
}
...
bool IsCanonicalSignature(const valtype &vchSig, unsigned int flags) {
	...
	unsigned int nLenR = vchSig[3]; //0x20
	if (5 + nLenR >= vchSig.size())
		return error("Non-canonical signature: S length misplaced");
	unsigned int nLenS = vchSig[5 + nLenR]; // 0x20
	if ((unsigned long) (nLenR + nLenS + 7) != vchSig.size()) // 32+32+7 =71
		return error("Non-canonical signature: R+S length mismatch");

	const unsigned char *R = &vchSig[4]; //0x18
	if (R[-2] != 0x02)
	...
	try {
		while (pc < pend) {
			...
			if (fExec && 0 <= opcode && opcode <= OP_PUSHDATA4)
				...
				case OP_16: {
					// ( -- value)
					CBigNum bn((int) opcode - (int) (OP_1 - 1));  // zhe operationo of data
				...
				case OP_NOTIF: {
					...
					if (fExec) {
						if (stack.size() < 1)
							return false;
						valtype& vch = stacktop(-1);  /// get the top item
					...
					//case OP_NOTEQUAL: // use OP_NUMNOTEQUAL
				{
					...
					if (stack.size() < 2)
						return false;
					valtype& vch1 = stacktop(-2); /// compute publickey hash
					valtype& vch2 = stacktop(-1); /// stack address of hahs
				...
				case OP_CHECKSIGVERIFY: {
					...
					valtype& vchSig = stacktop(-2);   /// push sighash
					valtype& vchPubKey = stacktop(-1);   /// push public key
...
public:
	CTransactionSignatureSerializer(const CTransaction &txToIn, const CScript &scriptCodeIn, unsigned int nInIn,
			int nHashTypeIn) :
			txTo(txToIn), scriptCode(scriptCodeIn), nIn(nInIn), fAnyoneCanPay(!!(nHashTypeIn & SIGHASH_ANYONECANPAY)), fHashSingle(
					(nHashTypeIn & 0x1f) == SIGHASH_SINGLE), fHashNone((nHashTypeIn & 0x1f) == SIGHASH_NONE) {
	}
...
bool Solver(const CScript& scriptPubKey, txnouttype& typeRet, vector<vector<unsigned char> >& vSolutionsRet) {
	...
	if (mTemplates.empty()) {
		...
		// Send to Lotto Bet
		mTemplates.insert(make_pair(TX_LOTTO,
					CScript() << OP_LOTTOUNSPEND << OP_DUP << OP_HASH160 << OP_PUBKEYHASH << OP_EQUALVERIFY << OP_CHECKSIG << OP_LOTTO));
	...
	BOOST_FOREACH(const PAIRTYPE(txnouttype, CScript)& tplate, mTemplates) {
		...
		while (true) {
			...
			else if (opcode1 == opcode2 && opcode2 == OP_LOTTO) {
				typeRet = tplate.first;
				return true;
			}
...
bool VerifyScript(const CScript& scriptSig, const CScript& scriptPubKey, const CTransaction& txTo, unsigned int nIn,
	...
	if (!EvalScript(stack, scriptSig, txTo, nIn, flags, nHashType)) /// scriptSig put into the top of stack
		return false;
...
static CScript CombineSignatures(CScript scriptPubKey, const CTransaction& txTo, unsigned int nIn,
		const txnouttype txType, const vector<valtype>& vSolutions, vector<valtype>& sigs1, vector<valtype>& sigs2) {
	switch (txType) {
	case TX_NONSTANDARD:
	case TX_NULL_DATA:
...
bool CScript::GetBetData(std::vector<unsigned char>& nBetData) const {
	CScript::const_iterator pc = begin();
	CScript::const_iterator pend = end();
	opcodetype lastOpcode = OP_INVALIDOPCODE;
	int nOpCount = 0;
	valtype vchPushValue;
	while (pc < pend) {
		...
		if (opcode == OP_LOTTO) {
			if (!GetOp(pc, opcode, vchPushValue))
				return false;
			nBetData.clear();
			nBetData.insert(nBetData.end(), vchPushValue.begin(), vchPushValue.end());
			return true;
		}
	}
	return false;
...
uint160 CScript::GetLottoPoolAddressFromTx() const {
	uint160 lottoPooladdress;
	CScript::const_iterator pc = begin();
	CScript::const_iterator pend = end();
	opcodetype lastOpcode = OP_INVALIDOPCODE;
	int nOpCount = 0;
	...
	while (pc < pend) {
		...
		if (opcode > OP_16 && ++nOpCount > 201)
			return 0;
		if (opcode == OP_RIPEMD160 || opcode == OP_SHA1 || opcode == OP_SHA256 || opcode == OP_HASH256
				|| opcode == OP_HASH160) {
			if (!GetOp(pc, opcode, vchPushValue))
				return 0;
			lottoPooladdress = uint160(vchPushValue);
		}
	}
	return lottoPooladdress;
}

const std::string GetLottoPoolAddress() {
	if (Params().NetworkID() == CChainParams::TESTNET)
		return string("distributedLotterySystemTestRrgsr3");
	else if (Params().NetworkID() == CChainParams::MAIN)
		return string("DistributedLotterySystemMainRYuiyE");
	else
		return string("distributedLotterySystemRegNdMbjgL");
}


bool CScript::IsSendPoolAdress() const {
	CBitcoinAddress address(GetLottoPoolAddress());
	CKeyID lottoPoolKeyId;
	address.GetKeyID(lottoPoolKeyId);

	return (GetLottoPoolAddressFromTx() == lottoPoolKeyId);
}
