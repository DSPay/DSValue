...
//	LogTrace("bitcoin-cli","boost::filesystem::path &GetDataDir(bool fNetSpecific)\n");
...
static std::string memdump(const unsigned char * const base, int size) {
	int i;
	std::string tep;
	char psz[20];
	unsigned int *p32 = (unsigned int *) base;
...
std::string VectorToString(const std::vector<unsigned char> & printvec) {
	...
	char temp2[50];
	sprintf((char*) temp2, "vec size:%d data:", printvec.size());
	return std::string(temp2) + memdump(&printvec[0], printvec.size());
}
...
string getvchSelect(int selectsize,int totalsize) {
