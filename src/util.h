...
static inline std::string  err(const char *sExpression,const char *file,int line)
{
	return  tfm::format("err:%s file:%s line:%d \n",sExpression,file,line);
}
//#define Assert(_Expression) \
// (void) \
// ((!!(_Expression)) || \
//  (throw std::invalid_argument("shit"),0))
#define Assert(_Expression) \
 (void) \
 ((!!(_Expression)) || \
  (throw std::runtime_error(err(#_Expression,__FILE__,__LINE__)),0))
class CNetAddr;
class uint256;
...
//#define LogPrintf(...) LogTrace("before", __FILE__, __FUNCTION__, __LINE__ , __VA_ARGS__)
